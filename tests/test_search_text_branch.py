...
def test_dedicated_text_branch_does_not_apply_siglip_ocr_confidence_threshold(monkeypatch):
    """Qwen and SigLIP cosine values are not on the same calibration scale."""
    from app import config, search
    ...
    image_cache = search._EmbedCache(
        ...
        text_count=0,
        media_ids=[1, 2],
        # Values measured for the visual leg of "синие волосы" on the
        # production index; they are below the old OCR-specific image floor.
        vecs=np.array([[0.1217, 0.0], [0.10, 0.0]], dtype=np.float32),
        text_media_ids=[],
        text_vecs=np.empty((0, 0), dtype=np.float32),
        meta={1: _meta(1), 2: _meta(2)},
    )
    text_cache = search._EmbedCache(
        ...
        media_ids=[],
        vecs=np.empty((0, 0), dtype=np.float32),
        # Qwen's valid scores use a substantially lower scale than SigLIP's
        # OCR-text vectors, so 0.4125 must not be compared to SigLIP's 0.82.
        text_media_ids=[1, 2],
        text_vecs=np.array([[0.4125, 0.0], [0.4068, 0.0]], dtype=np.float32),
        meta={1: _meta(1), 2: _meta(2)},
    )
